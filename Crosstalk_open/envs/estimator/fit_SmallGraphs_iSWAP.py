...
bw_size = 256
lr_decay = 0.98  # decay every 10 epochs

data_datetime_path = '0601-1958_/'

gate_type = 'iSWAP'

def fit_smallgraphs_iSWAP(seed=2, print_or_plot_or_savemodel=True):
    ...
    model = GBFCN2_iSWAP(dropout=0.5, hidden_layers=32)
    model.to(dev)

    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=0e-5)
    ...
    for epoch in range(381):
        ...
        for i in range(10):
            ...
            MSE_valid_this_epoch += torch.mean(torch.stack(loss_valid_datas[:3])).item()
            r2_valid_this_epoch += torch.mean(torch.stack(r2_valid_datas[:3])).item()
            MSE_valid_s12_this_epoch += torch.mean(torch.stack(loss_valid_datas[3:])).item()
            r2_valid_s12_this_epoch += torch.mean(torch.stack(r2_valid_datas[3:])).item()
    ...
    for i in range(10):
        ...
        MSE_test_this_epoch += torch.mean(torch.stack(loss_test_datas[:3])).item()
        r2_test_this_epoch += torch.mean(torch.stack(r2_test_datas[:3])).item()
        MSE_test_s12_this_epoch += torch.mean(torch.stack(loss_test_datas[3:])).item()
        r2_test_s12_this_epoch += torch.mean(torch.stack(r2_test_datas[3:])).item()
...
if __name__ == '__main__':
    outiSWAP = fit_smallgraphs_iSWAP(seed=17)

    torch.save(outiSWAP, saving_dir + f"data_outiSWAP.pt")
    print(outiSWAP[-4:])